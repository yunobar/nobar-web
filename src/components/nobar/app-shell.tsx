  ...
  return (
    <div className="flex min-h-screen flex-col nb-scroll">
      <header className="sticky top-0 z-30 border-b border-border bg-[color-mix(in_oklch,var(--bg)_82%,transparent)] backdrop-blur-md">
        <div className="mx-auto flex h-[60px] max-w-[1120px] items-center gap-5 px-6">
          <button
            onClick={() => navigate({ to: "/" })}
          ...
          {showWatchCTA && (
            <Link
              to="/watch"
              className="flex h-9 items-center gap-1.5 whitespace-nowrap rounded-lg bg-brand px-[15px] text-[13.5px] font-semibold text-brand-foreground shadow-[var(--shadow)] hover:opacity-90"
            >
              ✦ Watch together
            </Link>
          )}
          ...
          <button
            onClick={toggleTheme}
            title="Toggle theme"
            className="flex size-9 items-center justify-center rounded-lg border border-border bg-surface text-[15px] hover:bg-secondary"
          >
            {theme === "light" ? "☾" : "☀"}
          </button>
      ...
      <main className="mx-auto w-full max-w-[1120px] flex-1 px-6 py-8 pb-20">{children}</main>
